 ...
 *
 * Built on the Autocomplete plugin:
...
$.fn.extend({
	livesearch: function(url, options) {
		options = $.extend({
			...
			parse: function(data) {
				return $.map(eval(data), function(row) {
					return {
						data: row,
						value: row.title,
						result: row.title + " <" + row.path + ">"
					};
				});
			},
			formatItem: function(link) {
...
$.Autocompleter = function(input, options) {
	...
	function selectCurrent() {
		...
		// Follow link
		hideResultsNow();
		document.location = selected.data.path;
		return true;
	}
...
$.Autocompleter.Select = function (options, input, select, config) {
	...
	return {
		...
		show: function() {
			...
			// Added -pk
			if( options.center_results) {
				var input_width = $(input).width();
				var x_offset = (parseFloat(width) - parseFloat(input_width)) / 2;
